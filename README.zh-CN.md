...
Dashboard 采用与 GrowthOS-Go 一致的中性色与紫色视觉，支持浅色、暗色、系统主题和移动端导航。保留原密码与加密账本，已有设备无需重新初始化。

新增最多 20 组命名筛选视图、真正的明细分页与完整 CSV 导出、设备状态筛选与诊断、刷新失败/部分设备失败提示、可配置自动刷新，以及浏览器内的月度费用提醒。请求表可以查看完整字段和单条详情；历史请求范围仍由设备端留存决定。

概览支持完整 7 天 / 30 天及上月周期对比，包含模型费用变化归因。概览与设备页提供账本、心跳、新鲜度、定价和请求留存检查；数据缺失或费用仅为下界时，会明确限制比较。工作区设置还可切换标准 / 大字阅读。

本地运行、模块结构、扩展方式与验证说明见 [Dashboard 开发文档](docs/FRONTEND.md)。
...
- **CSP**：静态 Dashboard 使用严格 Content Security Policy，脚本仅允许本站资源，网络连接限制到本站、`raw.githubusercontent.com` 与 GitHub 官方备用读取域名 `api.github.com`，并禁用对象与表单提交；不加载第三方分析脚本。
...
运行 `usagemesh serve` 即可打开内置 Web 页面，无需 Node 或 Docker。支持额度快照、项目与会话账本、系统提醒、Codex/Claude 供应商配置预览与回滚、渠道质量分析及只读代理连接器。配置备份、密钥与本地管理事件不上传云端；v2.7.0 起，脱敏的官方额度与周期快照随加密设备账本同步。详细启动方法、密钥处理与数据边界见[本地工作台指南](docs/LOCAL-WORKSPACE.md)。

## 官方订阅周期（v2.7.0 起）

云端“周期用量”按官方重置时间汇总所有已读取设备的 Codex 官方订阅 Tokens、请求数与价卡费用估算。额度百分比是账号与额度类别的最新快照，不跨设备相加；API 付费、中转与来源不明记录不计入。分钟桶跨越精确重置边界时单独提示待核对。至少一台设备升级并同步周期信息即可选择窗口，其他设备已有的官方订阅分钟记录可继续聚合。

本地工作台通过已登录的 `codex app-server` 只读采样，记录重置周期与历史观测；官方每日 Tokens 独立展示，不按日汇总反推精确周期用量。未采集的历史无法自动补齐，历史百分比代表最后观测值。详见[周期口径与覆盖范围](docs/LOCAL-WORKSPACE.md#官方订阅周期v270-起)。

周期预测还提供消耗速度、预计耗尽时间、重置时用量、可持续速度与趋势图；Tokens 容量通过手动校准的情景估算单独展示。方法和开源参考见[额度预测说明](docs/QUOTA-FORECAST.md)。
